	...
	if(inputsname!=''){
		...
		$.each(inputsname.split('|'), function(i, value) {
			if(value=='说说ID'||value=='说说ＩＤ')
				var addstr='<br /><div class="btn btn-default btn-round btn-block" onclick="get_shuoshuo(\'inputvalue'+(i+2)+'\',$(\'#inputvalue\').val())">点击这里自动获取说说ＩＤ</div>';
			else
				var addstr='';
			$('#inputsname').append('<div class="form-group"><label for="text" id="inputname">'+value+'</label><input type="text" name="inputvalue'+(i+2)+'" id="inputvalue'+(i+2)+'" value="" class="form-control" required/>'+addstr+'</div>');
		});
		$('#inputsname').show();
	}else{
		$('#inputsname').html("");
...
$("#inputvalue").blur(function () {
	if($("#inputname").html() == '快手ID'||$("#inputname").html() == '快手ＩＤ'){
		if($("#inputvalue").val()!='' && isNaN($("#inputvalue").val())){
			getkuaishouid();
		}
	}
	if($("#inputname").html() == '歌曲ID'||$("#inputname").html() == '歌曲ＩＤ'){
		...
		success : function(data) {
			...
			if(data.code == 0){
				...
				$.each(data.data, function (i, res) {
					$("#tid").append('<option value="'+res.tid+'" cid="'+res.cid+'" price="'+res.price+'" alert="'+res.alert+'" inputname="'+res.input+'" inputsname="'+res.inputs+'" multi="'+res.multi+'">'+res.name+'</option>');
					num++;
				});
				$("#tid").val(0);
				getPoint();
			...
			success : function(data) {
				if(data.code == 0){
					...
					if(inputsname!=''){
						...
						$.each(inputsname.split('|'), function(i, value) {
							if(value=='说说ID'||value=='说说ＩＤ')
								var addstr='<br /><div class="btn btn-default btn-round btn-block" onclick="get_shuoshuo(\'km_inputvalue'+(i+2)+'\',$(\'#km_inputvalue\').val(),1)">点击这里自动获取说说ＩＤ</div>';
							else
								var addstr='';
							$('#km_inputsname').append('<div class="form-group"><label for="text" id="km_inputname">'+value+'</label><input type="text" name="inputvalue'+(i+2)+'" id="km_inputvalue'+(i+2)+'" value="" class="form-control" required/>'+addstr+'</div>');
						});
						$('#km_inputsname').show();
					}else{
						$('#km_inputsname').html("");
