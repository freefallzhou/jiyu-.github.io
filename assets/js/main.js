	...
	if(inputsname!=''){
		...
		$.each(inputsname.split('|'), function(i, value) {
			if(value=='说说ID'||value=='说说ＩＤ')
				var addstr='<span class="input-group-btn"><div class="btn btn-primary" onclick="get_shuoshuo(\'inputvalue'+(i+2)+'\',$(\'#inputvalue\').val())">点击自动获取</div></span>';
			else if(value=='作品ID'||value=='作品ＩＤ')
				var addstr='<span class="input-group-btn"><div class="btn btn-primary" onclick="get_kuaishou(\'inputvalue'+(i+2)+'\',$(\'#inputvalue\').val())">点击自动获取</div></span>';
			else
				var addstr='';
			...
			if(data.code == 0){
				...
				if($('#show_shuoshuo').length > 0){
					...
					if(km==1){
						...
						$('#inputsname').append('<div class="form-group"><div class="input-group"><div class="input-group-addon">选择说说</div><div id="show_shuoshuo"><select id="shuoid" class="form-control" onchange="set_shuoshuo(\''+id+'\');">'+addstr+'</select></div>');
					}
				}
				set_shuoshuo(id);
			}else{
